...
% Number of users at each layout is from 1000 to 10000
% Each user has 2 to 6 frequently visited places
% k=16, r=16
% Quota is 1000
% 400 Servers
% we set 100 samples
...
clc;clear;
rng('shuffle');
sampleSize = 2; % the layout size
rngSeed = randperm(10000,sampleSize);
S = struct();
quota = 100;
StorageNodeSize = 400; %storage node size
LocationSize = 200;  % the number of locations, we want to generate
LocationScale = 10000; % the size of layout
LocationDist = 10;      % min distance between any a pair of locations

numberOfUsers = 10000; % the number of users in each layout
...
maxTransferSpeed = 125; %MB
minTransferSpeed = 3; %MB

%%%%%%%%%%%%%%%

BoneNodeSize = 20;
BoneEdgeSize = 30;
ApNodeSize = 500;        % number of base stations
...
UserLocationMaxDist = 100000;%1000;   the picked locations should qualify the constraint, i.e., no greater than 3000
Test = 1;
...
for ii=1:sampleSize
    ...
    [speed,locMaxSpeedArray] = GenerateSpeedMatrix(FA, FC, StorageBegin, StorageEnd, LocationBegin, LocationEnd,  minTransferSpeed, maxTransferSpeed , 0);
    speed =speed';
    ...
    S(ii).LocationMaxSpeed = locMaxSpeedArray;
...
filenamePrefix = sprintf('largeScale1_%d_S%d_L%d_U%d', sampleSize, StorageNodeSize, LocationSize, numberOfUsers);
