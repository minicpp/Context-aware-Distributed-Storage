%experiment:
%Keep k=4, then adjust r=2~8
%Keep r=4, then adjust k=2~8
%Keep Ratio 1:1, then k or r = 2~6
%This is for small scale experiment
%Quota is 3
%25 Servers
%4 Users, the freq place of user is in 2~3 randomly
%To make it faster, we set 30 samples

clc;clear;
rng('shuffle');
sampleSize = 100; % the layout size
rngSeed = randperm(10000,sampleSize);
S = struct();
quota = 3;
StorageNodeSize = 25; %storage node size
LocationSize = 30;  % the number of locations, we want to generate
LocationScale = 2000; % the size of layout
LocationDist = 15;      % min distance between any a pair of locations

numberOfUsers = 4; % the number of users in each layout
minVisitedLocations = 2;
maxVisitedLocations = 4;
...
filenamePrefix = sprintf('smallScale1_%d_S%d_L%d_U%d', sampleSize, StorageNodeSize, LocationSize, numberOfUsers);
